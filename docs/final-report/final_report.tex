...
From an engineering perspective, we also wanted to master language design and memory management by building every component manually, without relying on third-party libraries.
...
\begin{figure}[!h]
    \centering
    \includegraphics[width=1\linewidth]{images/sysArc.png}
    \caption{System Architecture Flow: From Source Code to Execution}
\end{figure}

The following features were implemented:
\begin{itemize}
    \item \textbf{Input/Output:} Accepts user input through the \texttt{drimIn()} command and shows output using \texttt{wakeOut()}.
    ...
    \item \textbf{Mathematical Operations:} Full support for math like addition (+), subtraction (-), multiplication (*), division (/), modulo (\%), and power ($\wedge$).
    ...
    \item \textbf{Control Flow:} Supports conditional logic (if-else) and loops (\texttt{drimming}).
    ...
    \item \textbf{Zero-Dependency Architecture:} Built from scratch in C++ 17 without using any external libraries for parsing.
...
The following class diagram represents the structure and relationships between the different components of the Drim language:

\begin{figure}[!h]
    \centering
    \includegraphics[width=1\linewidth]{images/classDiagram.png}
...
\begin{itemize}
    \item \textbf{Single Responsibility Principle (SRP):} Each core component is dedicated to a specific task. The \texttt{Lexer} is responsible solely for tokenization, the \texttt{Parser} for syntax validation and AST construction, and the \texttt{Interpreter} for logic execution. This separation ensures that changes to the grammar do not affect the execution engine.
...
\begin{itemize}
    ...
    \begin{itemize}
        \item Created basic input and output systems.
        ...
        \item Fixed modulo operations for math.
    \end{itemize}
    ...
    \begin{itemize}
        ...
        \item Implemented data structure support.
        \item Developed string interpolation and formatted output.
